...
TARGETZIPDIR="/var/www/" # If valid, output zip will be moved there from default TARGETDIR location
...
for ARG in "$@"; do
	case "$ARG" in
		...
		--bare|bare) BARE=1 ;;
		--all|all) ALL=1 ;;
		--version=*) TARGETZIPNAME="FALCON_V$(echo "$ARG" | cut -d '=' -f2)" ;;
		*) TARGETCONFIGS+=("$ARG") ;;
	esac
done

PREFIXES="arm-linux-gnueabi arm-eabi" # Valid prefixes used by Linaro and Google
